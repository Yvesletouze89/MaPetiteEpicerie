		...
		if (isset($_POST['email']) &&  isset($_POST['password']))
		{
			...
			if ($user->checkEmail($_POST['email'])) 
			{
				...
				if (sha1($_POST['password']) == $utilisateur['password'])
				{
					...
					$this->show("default/accueil", ["message" => "Vous etes connecté !", "prenom"=> $utilisateur['prenom'], "nom"=> $utilisateur['nom']]);
				}
				else
				{
...
$this->show("users/connect", ["message" => "Votre mot de passe a bien ete changé !"]);
						
				    }else{$this->show("users/new-password",["message" => "champ vide !"]);}
				    
				}
			}else{
				    $this->show("users/new-password", ["message" => "les mots de passe ne correspondent pas !"]);
			}
