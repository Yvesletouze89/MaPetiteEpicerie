...
if (!empty($_POST['nom']) && !empty($_POST['prenom'])
 ...
 && !empty($_POST['cp'])  
 && !empty($_POST['mobile']) && !empty($_POST['heure_pref']))
//ville et addresse	
{
	...
	$adress2 = htmlspecialchars(htmlentities(strip_tags($_POST['adress2'])));
	...
	$heure_pref = htmlspecialchars(htmlentities(strip_tags($_POST['heure_pref'])));
		...
		$password = sha1($password);
			...
			// Insert le nouvel utilisateur 	 
			{
				...
				$query = $db->prepare("INSERT INTO utilisateurs (nom, prenom, adresse1, adresse2, CP, ville, tel, mobile, heure_pref) VALUES(?,?,?,?,?,?,?,?,?)");	
				$query->bindValue(1,  $nom, PDO::PARAM_STR);
				$query->bindValue(2,  $prenom, PDO::PARAM_STR);
				$query->bindValue(3,  $adress, PDO::PARAM_STR);
				$query->bindValue(4,  $adress2, PDO::PARAM_STR);
				$query->bindValue(5,  $CP, PDO::PARAM_INT);
				$query->bindValue(6,  $ville, PDO::PARAM_STR);
				$query->bindValue(7,  $tel, PDO::PARAM_STR);
				$query->bindValue(8,  $mobile, PDO::PARAM_STR);
				$query->bindValue(9,  $heure_pref, PDO::PARAM_STR);
...
//ok ici
				...
				echo "<br /><a href='accueil.php'>Retour sur Accueil</a>";
